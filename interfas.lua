...
-- Variables to store saved location
local savedLocation = nil

-- Function to save the player's current location
...
local function teleportToLocation()
    if savedLocation and game.Players.LocalPlayer and game.Players.LocalPlayer.Character then
        local character = game.Players.LocalPlayer.Character
        character.HumanoidRootPart.CFrame = CFrame.new(savedLocation)
        print("Teleported to saved location: " .. tostring(savedLocation))
    else
        print("No location saved to teleport to.")
    end
end

-- Adding the buttons to the interface
RayfieldLibrary:CreateButton({
    Name = "Guardar Ubicación",
    Callback = saveLocation
})

RayfieldLibrary:CreateButton({
    Name = "Teletransportar a Ubicación Guardada",
